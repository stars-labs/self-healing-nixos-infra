...
```mermaid
flowchart LR
    subgraph Without["不协调"]
        A[PostgreSQL<br/>写事务 1] -->|已捕获| B[Btrfs 快照<br/>事务 1 OK]
        C[PostgreSQL<br/>写事务 2 进行中] -.->|半捕获| D[Btrfs 快照<br/>事务 2 损坏]
    end
    
    subgraph With["协调后"]
        E[PostgreSQL<br/>CHECKPOINT] -->|刷新到磁盘| F[Btrfs 快照]
        E -->|标记备份| F
        F -->|快照已拍摄| G[一致的快照]
    end
```
